    ...
    .then(function (productList) {
      ...
      for (let i in cart) {
        ...
        for (let i in productList) {
          if (cartItemId == productList[i]._id) {
            //si l'id d'un des produit correspond à celle de l'élément du cart
        ...
        var cart__item__content__settings__quantity =
        ...
        qtyNumber.addEventListener("change", (event) => {
          ...
          var toUpdateIndex = storedCart.findIndex(
            (element) =>
              element.id === cartItem.id &&
              element.chosenColor === cartItem.chosenColor
          );
          ...
          displayPriceElem = event.target
            .closest("article")
            .querySelector(".cartItemDisplayedPrice");
          displayPriceElem.innerHTML =
            cartItem.price * cartItem.quantity + " €";
          //recalcul du prix du panier
          document.getElementById("totalPrice").innerHTML = sumArray(cartItems);
        });

        var cart__item__content__settings__delete =
          document.createElement("div");
...
function orderRequest(e) {
  //vérifie les différent champ et si ok, construit la requête pour la commande, l'envoi et redirige vers la page de commande
  e.preventDefault();
  if (
    checkFirstName() &&
    checkLastName() &&
    ...
    var cart = JSON.parse(localStorage.getItem("cart"));
    for (i in cart) {
      //pour chaque élément du cart

      var productId = null;
      for (qty = 1; qty <= cart[i].quantity; qty++) {
        //autant de fois qu'il y a de quantité de l'article
    ...
    if (productTable.length != 0) {
      ...
      alert(
        "Vous ne pouvez pas commander, il n'y aucun article dans le panier"
      );
    }
  }
}
